...
// Subscriptions
export function subscribeClearanceRequests(onUpdate: (requests: ClearanceRequest[]) => void) {
  ensureInitialSeed();
  ...
  return onSnapshot(
    ...
    (error) => {
      console.warn('Fallback to memory requests due to snapshot error:', error);
      onUpdate(INITIAL_REQUESTS);
      handleFirestoreError(error, OperationType.GET, 'clearance_requests');
    }
  );
}

export function subscribeAuditLogs(onUpdate: (logs: AuditRecord[]) => void) {
  ...
  return onSnapshot(
    ...
    (error) => {
      console.warn('Fallback to memory audit logs:', error);
      onUpdate(INITIAL_AUDIT_LOGS);
...
export function subscribeCertificates(onUpdate: (certs: DigitalCertificate[]) => void) {
  ...
  return onSnapshot(
    ...
    (error) => {
      console.warn('Fallback to memory certificates:', error);
      onUpdate(INITIAL_CERTIFICATES);
...
export async function createClearanceRequest(
  ...
  try {
    ...
    handleFirestoreError(error, OperationType.WRITE, 'clearance_requests');
  }
...
export async function updateDepartmentAction(
  ...
  try {
    ...
    handleFirestoreError(err, OperationType.UPDATE, `clearance_requests/${request.id}`);
  }
}
